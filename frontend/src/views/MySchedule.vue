            ...
            <div 
              ...
              <div class="schedule-details">
                ...
                <div class="detail-row">
                  ...
                  <span class="detail-value">{{ schedule.timeRange }}</span>
                </div>
                <div class="detail-row">
                  <span class="detail-label">预约：</span>
                  <span class="detail-value">{{ schedule.patientCount }} / {{ schedule.maxPatients }} 人</span>
                </div>
              </div>
              <button @click="viewScheduleDetail(schedule)" class="view-btn">
                查看详情
              </button>
            </div>
          </div>
...
async function fetchTodaySchedule() {
  ...
  try {
    // 实际使用时替换为：
    // const { data } = await axios.get('/api/doctor/schedule/today')
    ...
    todaySchedule.value = mockData
  } catch (err) {
    console.error('获取今日排班失败', err)
  } finally {
    loading.value = false
  }
}
...
async function fetchFutureSchedules() {
  ...
  try {
    // 实际使用时替换为：
    // const { data } = await axios.get('/api/doctor/schedule/future')
    ...
    futureSchedules.value = mockData
  } catch (err) {
    console.error('获取未来排班失败', err)
  } finally {
    loading.value = false
  }
}
...
function viewScheduleDetail(schedule) {
  alert(`查看 ${schedule.date} 的详细排班信息`)
  // 实际可跳转到详情页或显示弹窗
}
